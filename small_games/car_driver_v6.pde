...
// 🔥 CONTROL TRAFFIC HERE
int TRAFFIC_DENSITY = 80;
...
void generateWorld() {
  ...
  for (int x = 0; x < worldSize; x += roadSpacing) {
    for (int y = 0; y < worldSize; y += roadSpacing) {
      ...
      float cx = x + roadSpacing/2;
      float cy = y + roadSpacing/2;

      float w = roadSpacing - roadWidth;
      float h = roadSpacing - roadWidth;

      houses.add(new House(cx, cy, w, h));
    }
  }
